...
export const getPayments = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const enrichedPayments = payments.map((p) => ({
      ...
      fee: p.contract ? Math.round(p.totalAmount * (p.contract.platformFee / 100)) : 0,
      netAmount: p.contract ? p.totalAmount - Math.round(p.totalAmount * (p.contract.platformFee / 100)) : p.totalAmount,
      status: p.status,
      method: p.method,
