...
interface NavItem {
  ...
  badgeParam?: string; // 배지>0 클릭 시 붙일 쿼리 → 해당 페이지가 '미처리' 필터로 진입
  badgeDesc?: string;  // 배지 툴팁(숫자가 뭘 세는지 설명)
}
...
const navGroups: NavGroup[] = [
    ...
    items: [
      {
        ...
        href: "/caregivers",
        badgeKey: "caregivers",
        badgeParam: "status=PENDING",
        badgeDesc: "승인 대기 간병인 수",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        href: "/disputes",
        badgeKey: "disputes",
        badgeParam: "status=todo",
        badgeDesc: "미처리 분쟁(접수·처리중) 수",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
      ...
      },
      {
        ...
        badgeKey: "businessInquiries",
        badgeParam: "status=PENDING",
        badgeDesc: "미확인 제휴 문의 수",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
    ...
    items: [
      {
        ...
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6.22 7.548M14.857 17.082a23.848 23.848 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0M3 3l18 18" />
          </svg>
        ),
      },
      {
        label: "알림톡 템플릿",
        href: "/alimtalk-templates",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
  ...
  },
  {
    ...
    items: [
      ...
      {
        label: "직접결제 요청",
        href: "/direct-payments",
        badgeKey: "directPayments",
        badgeParam: "status=PENDING",
        badgeDesc: "이용료 입금확인 대기 건수",
        icon: (
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
    ...
    const load = async () => {
      try {
        ...
        setBadges({
          ...
          directPayments: data.directPayments || 0,
          businessInquiries: data.businessInquiries || 0,
        });
      } catch {
        // ignore
      }
    ...
    <>
          ...
          <div key={groupIdx} className={groupIdx > 0 ? "mt-4" : ""}>
            ...
            <ul className="space-y-0.5">
              {group.items.map((item) => {
                ...
                // 배지가 있으면 '미처리' 필터 파라미터를 붙여 이동 → 페이지 목록 = 배지 숫자
                const linkHref =
                  badgeCount > 0 && item.badgeParam ? `${item.href}?${item.badgeParam}` : item.href;
                return (
                  <li key={item.href}>
                    <Link
                      href={linkHref}
      ...
      <div className="border-t border-white/10 px-4 py-3">
        ...
        >
          ...
          <span>홈페이지로 이동</span>
          <svg className="ml-auto h-3 w-3 opacity-60" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
