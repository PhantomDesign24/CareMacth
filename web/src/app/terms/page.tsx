// 이용약관 — 관리자 CMS(DB) 내용이 있으면 그걸 렌더, 없으면 아래 기본(정적) 내용 폴백
  ...
  if (doc?.content) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-10 sm:py-16 md:py-24">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-6 sm:mb-8">{doc.title || "이용약관"}</h1>
        {doc.effectiveDate && (
          <p className="text-sm text-gray-400 mb-6">시행일: {doc.effectiveDate}</p>
        )}
        <div
          className="prose prose-gray max-w-none space-y-8 [&_h2]:text-xl [&_h2]:font-semibold [&_h2]:text-gray-900 [&_h2]:mb-3 [&_p]:text-gray-600 [&_p]:leading-relaxed [&_ul]:list-disc [&_ul]:list-inside [&_ul]:text-gray-600 [&_ul]:space-y-2 [&_section]:block"
