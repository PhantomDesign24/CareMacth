...
// 주: 폼/백엔드의 region 표기와 통일하기 위해 시·도는 짧은 표기 사용 (서울/경기 등)
...
// "서울 강남구" → "서울"
// "서울"      → "서울"
export function extractSido(region: string): string | null {
  if (!region) return null;
  const head = region.trim().split(/\s+/)[0];
  return KOREA_REGIONS[head] ? head : null;
}

// "서울 강남구" → "강남구"
// "서울"        → null
export function extractSigungu(region: string): string | null {
  if (!region) return null;
...
// 주어진 region 배열에서 가장 많이 등장하는 시·도 (없으면 null)
export function dominantSido(regions: string[]): string | null {
  if (!Array.isArray(regions) || regions.length === 0) return null;
  const counts: Record<string, number> = {};
  for (const r of regions) {
    const s = extractSido(r);
